...
#define VM_EMULATED_INSTRUCTION_IO_SINGLE(direction, size)                                             \
	void vmx_emulated_instruction_##direction##size(vmx_vmexit_state_t* vm_state, uint16_t port) { \
		vmx_##direction##size(port, vm_state->reg_state->rax);                                 \
	}
...
#define VM_EMULATED_INSTRUCTION_IO_STR(direction, size, type)                                             \
	void vmx_emulated_instruction_##direction##s##size(vmx_vmexit_state_t* vm_state, uint16_t port) { \
		bool df = vm_state->rflags & RFLAGS_DF;                                                   \
                                                                                                          \
		/* TODO : introduce bound checks when EPT is used */                                      \
		vmx_##direction##size(port, *((type*)vm_state->reg_state->rsi));                          \
...
#define VM_EMULATED_INSTRUCTION_IO_STRREP(direction, size)                                                    \
	void vmx_emulated_instruction_rep_##direction##s##size(vmx_vmexit_state_t* vm_state, uint16_t port) { \
		while (vm_state->reg_state->rcx != 0) {                                                       \
			vmx_emulated_instruction_##direction##s##size(vm_state, port);                        \
